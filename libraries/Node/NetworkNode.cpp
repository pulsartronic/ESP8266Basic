...
void NetworkNode::applySettings() {
	...
	this->httpServer = new ESP8266WebServer(this->settings.httpPort);
...
void NetworkNode::httpResponse(JsonObject& response) {
	String responseSTR = "";
	serializeJson(response, responseSTR);
	String edata = this->aesm->encrypt((byte*) responseSTR.c_str(), responseSTR.length());
